...
def handle_identity_command(args):
    ...
    if args.identity_command == 'add':
        ...
        if add_identity_mapping(args.repo_path, args.name_or_email, args.canonical_identity):
        ...
        if remove_identity_mapping(args.repo_path, args.name_or_email):
            print(f"{Fore.GREEN}Successfully removed identity mapping for {args.name_or_email}.{Style.RESET_ALL}")
        else:
        ...
        if not mappings['canonical_names'] and not mappings['canonical_emails']:
            print(f"{Fore.YELLOW}No identity mappings found for {args.repo_path}.{Style.RESET_ALL}")
            print(f"Use 'gitstats identity add <repo_path> <name_or_email> <canonical_identity>' to add a mapping.")
            return
        ...
        if mappings['canonical_names']:
            ...
            name_table = [[name, canonical] for name, canonical in mappings['canonical_names'].items()]
            print(tabulate(name_table, headers=["Name", "Canonical Identity"], tablefmt="grid"))
...
def setup_identity_parser(subparsers):
    ...
    add_parser.add_argument(
        'name_or_email',
        help='Author name or email address to map'
    )
    add_parser.add_argument(
        'canonical_identity',
        help='Canonical identity to map to'
    )
    
    # Remove command
    remove_parser = identity_subparsers.add_parser(
        'remove',
        help='Remove an identity mapping'
    )
    ...
    remove_parser.add_argument(
        'name_or_email',
        help='Author name or email address to remove mapping for'
    )
    
    # List command
    list_parser = identity_subparsers.add_parser(
        'list',
        help='List all identity mappings for a repository'
    )
